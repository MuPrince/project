...
import com.prince.annotion.Excel;
import com.prince.exception.ExcelException;
...
public class ExcelUtil<T> implements Serializable {
     ...
     *            java输入流
     */
    public List<T> importExcel(String sheetName, InputStream input) throws ExcelException {
     ...
     *            java输出流
     */
    public boolean exportExcel(List<T> list, String sheetName, OutputStream output) throws ExcelException {
        try {
                        ...
                        for (int j = 1, len = (sheet.getLastRowNum() - 1); j < len; j++) {
                            ...
                            if (hssfRow != null) {
                                ...
                                if (hssfCell != null && hssfCell.getCellType() == CellType.STRING
                                    ...
                                    totalNumber = new BigDecimal(hssfCell.getStringCellValue());
                                }
                            }
                        }
...
//            user.setGender(gender[new Random().nextInt(2)]);
//            user.setTelephone(TelephoneNumberUtil.getTel());
//            list.add(user);
//        }
//        System.out.println(list);
//        FileOutputStream out = new FileOutputStream("D:\\test.xls");
//        ExcelUtil<User> util1 = new ExcelUtil<User>(User.class);
//        util1.exportExcel(list, "test信息", out);
//        System.out.println("----执行完毕----");

//        FileInputStream fis = new FileInputStream("D:\\test.xls");
//        ExcelUtil<User> util1 = new ExcelUtil<User>(User.class);
//        List<User> testModels = util1.importExcel("test", fis);
//        for (User testModel : testModels) {
